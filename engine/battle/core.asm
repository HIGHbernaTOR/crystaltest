...
ApplyExperienceAfterEnemyCaught:
	...
	; Give 100% EXP individually to every participating Pokémon.
	...
	; Non-participants receive 50% of the normal EXP.
	ld hl, wEnemyMonBaseExp
	srl [hl]
    ; Temporarily replace the participant flags with the inverse:
	; every non-participant becomes eligible.
	ld a, [wBattleParticipantsNotFainted]
	push af
	ld a, d
	xor %00111111
...
.stat_exp_awarded
	...
	; Do not award regular EXP at level 100.
	; Level 100 Pokémon still receive Stat Exp,
	; but do not receive regular EXP.
	pop bc
	ld hl, MON_LEVEL
	add hl, bc
	ld a, [hl]
